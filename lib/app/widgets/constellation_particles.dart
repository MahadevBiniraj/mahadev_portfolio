...
class ConstellationParticles extends StatefulWidget {
  const ConstellationParticles({super.key, this.particleCount = 40});
...
class _ConstellationPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final p in particles) {
      ...
      // Simple glow for larger particles (cheaper than radial gradient)
      if (p.radius > 1.2) {
        _particlePaint.color = particleColor.withValues(alpha: p.opacity * 0.15);
        canvas.drawCircle(Offset(p.x, p.y), p.radius * 3, _particlePaint);
      }
    }
  }
